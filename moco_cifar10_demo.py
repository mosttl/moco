...
"""### Set arguments"""
print()
print("The working dir: ", os.getcwd())
print()
...
# utils
parser.add_argument('--resume', default='/gemini/code/output/model_last.pth', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
parser.add_argument('--results-dir', default='/gemini/code/output', type=str, metavar='PATH', help='path to cache (default: none)')
...
# data prepare
train_data = CIFAR10Pair(root='/gemini/data-1/cifar-10-python', train=True, transform=train_transform, download=True)
train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)

memory_data = CIFAR10(root='/gemini/data-1/cifar-10-python', train=True, transform=test_transform, download=True)
memory_loader = DataLoader(memory_data, batch_size=args.batch_size, shuffle=False, num_workers=16, pin_memory=True)

test_data = CIFAR10(root='/gemini/data-1/cifar-10-python', train=False, transform=test_transform, download=True)
...
def mixup_loss(enc, x, alpha=1.0, use_cuda=True):
    '''Returns mixup loss'''
    if alpha > 0:
    ...
    #rep2 = rep[index, :]
    rep2 = enc(x2)
    rep_mixed = lam * rep + (1 - lam) * rep2

    mixed_rep = enc(mixed_x)

    pdist = nn.PairwiseDistance(p=2).cuda()
    output = pdist(rep_mixed, mixed_rep)

    loss = torch.sum(output) / batch_size
...
def train(net, data_loader, train_optimizer, epoch, args):
    ...
    for im_1, im_2 in train_bar:
        ...
        loss = net(im_1, im_2)
        loss_mixup = mixup_loss(model.encoder_q, im_1, alpha=1.0, use_cuda=use_cuda)
        # print("Contrastive Loss: ", loss)
        # print("Mixup Loss: ", loss_mixup)
